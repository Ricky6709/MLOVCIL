...
from c1 import CustomCocoDataset
...
def create_incremental_tasks(data_dir, labeled_number, n, batch_size=64):
    ...
    ann_file = f'./annotations/instances_train2014.json'
    ...
    class IncrementalCocoDataset(CustomCocoDataset):
        ...
        def __getitem__(self, index):
            img, classes = super(IncrementalCocoDataset, self).__getitem__(index)
            if -1 in classes:
                return img, None
            else:
                new_targets = [label for idx, label in classes]
...
if __name__ == "__main__":
    ...
    # 保存任务 1 和任务 2 的索引
    save_indices_to_npy(task1_indices, 'task1t')
    save_indices_to_npy(task2_indices, 'task2t')
