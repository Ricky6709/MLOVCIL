...
def train_first(train_loader, epochs):
    ...
    for epoch in range(epochs):
        ...
        for (imgs, labels) in tqdm(train_loader, desc="Train"):
            ...
            # labels = labels.float()
            # pseudo_label = pseudo_label + labels
            # pseudo_label = (pseudo_label >= threshold).float()
            # loss = l2_loss(fin_logits, pseudo_label.float())

            # VLPL_loss
            threshold = 0.9

            # 获取 labels 中第21到40个类别中标签为1的所有伪标签概率
            # label_range = labels[:, 20:40]
            # label_mask = (label_range == 1).float()
            # pseudo_label_probs = sigmoid(ram_logits[:, 20:40].detach())

            # 计算这些概率的均值(最大值），忽略未标记的类别
            # if label_mask.sum() > 0:
            ...
            pseudo_label = (pseudo_label >= threshold).float()
            labels = labels.float()
            # pseudo_label[:, 20:40] = labels[:, 20:40]
...
if __name__ == "__main__":
    ...
    train_first(train_loader=loader2, epochs=args.epochs)