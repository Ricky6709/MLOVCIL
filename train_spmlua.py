...
def load_spml_datasets(
    ...
    if dataset == "voc":
        ...
        train_dataset = COCO2014_handler(X=np.load(os.path.join('./datasets/coco/npy', 'formatted_task1t_images.npy')),
                                         Y=np.load(os.path.join('./datasets/coco/npy', 'formatted_task1t_labels.npy')),
                                         data_path='./datasets/coco',
                                         input_size=input_size)
        test_dataset = COCO2014_handler(X=np.load(os.path.join('./datasets/coco/npy', 'formatted_task1v_images.npy')),
...
def load_ram(                             # 使用clip把标签转换成label_embedding
        backbone: str,
        checkpoint: str,
        input_size: int,
        taglist: List[str],
...
def test_model(ram_model, cla_model, cla_img_model, cla_gcn_model, test_loader, imglist, taglist):
    ...
    targs = torch.empty(len(imglist), len(taglist))
    pos = 0
...
if __name__ == "__main__":
    ...
    # output_dir = args.output_dir + "/" + args.dataset + "/" + "Train_with_all_single_positive_labels_L2loss_eval-GCN"
    # output_dir = args.output_dir + "/" + args.dataset + "/" + "Train_with_all_single_positive_labels_EMloss"
    ...
    test_loader, test_info = load_spml_datasets(
        dataset=args.dataset,
        model_type=args.model_type,
        pattern="task1v",
        input_size=args.input_size,
    ...
    test_taglist, test_imglist, test_tag_des = \
        test_info["taglist"], test_info["imglist"], test_info["tag_des"]
    ...
    for epoch in range(args.epochs):
        ...
        # step_lr_schedule(optimizer, epoch, init_lr=1e-2, min_lr=5e-4, decay_rate=0.9)
        ...
        for (imgs, labels) in tqdm(loader, desc="Train"):
            ...
            loss = l2_loss(fin_logits, pseudo_label)
        ...
        mAP, APs = test_model(ram_model, cla_model, cla_img_model, cla_gcn_model, test_loader, test_imglist,
                              test_taglist)
