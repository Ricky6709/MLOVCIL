...
@torch.no_grad()
def test_model(ram_model, cad_model, test_loader, imglist,  taglist):
    ram_model.eval()
    cad_model.eval()
    ...
    for (imgs, labels) in tqdm(test_loader, desc="Test"):
        ...
        fin_logits = cad_model(tagging_embed)
...
def pseudo_label_generation(ram_model, cad_model, imgs):
...
def train_task1(train_loader, epochs):
    ...
    for epoch in range(epochs):
        ...
        for (imgs, labels) in tqdm(train_loader, desc="Train"):
            ...
            # print(tagging_embed.shape)
            fin_pres = cad_model(tagging_embed)
        ...
        mAP, APs = test_model(ram_model_1, cad_model, test1_loader, test1_imglist, test1_taglist)
...
def train_task2(train_loader, epochs):
    ...
    optimizer = torch.optim.AdamW(
        list(cad_model_2.parameters()), lr=1e-4, weight_decay=1e-4)

    ram_model_1.to(device).eval()
    ram_model_2.to(device).eval()
    cad_model.to(device).eval()
    cad_model_2.to(device).train()
    ...
    for epoch in range(epochs):
        ...
        for (imgs, labels) in tqdm(train_loader, desc="Train"):
            ...
            fin_logits = cad_model_2(tagging_embed)
            fin_pres = sigmoid(fin_logits).float()
      
            pseudo_labels_fin = pseudo_label_generation(ram_model_1, cad_model, imgs)
            student_subset = fin_logits[:, :20]
            loss_kd = mse_loss(pseudo_labels_fin, student_subset)
        ...
        # test and save checkpoint
        mAP, APs = test_model(ram_model_2, cad_model_2, test2_loader, test2_imglist, test2_taglist)

        if mAP >= best_mAP:
            save_obj = {
                'cad_model_2': cad_model_2.state_dict(),
                'epoch': epoch,
                'mAP': mAP,
                'APs': APs
            }
...
if __name__ == "__main__":
    ...
    cad_model = ClassAttentionDecoder.ClassAttentionDecoder(num_classes=20, embedding_dim=768, num_heads=8, num_blocks=4)
    ...
    # 加载模型权重
    cad_model.load_state_dict(checkpoint['cad_model'])

    # set up output paths
    output_dir_2 = args.output_dir + "/" + "coco_task2" + "/" + "Train_with_pseudo_labels_cad_val_VLPLloss"
    ...
    num_classes = len(taglist2)
    cad_model_2 = ClassAttentionDecoder.ClassAttentionDecoder(num_classes=num_classes, embedding_dim=768, num_heads=8, num_blocks=4)
